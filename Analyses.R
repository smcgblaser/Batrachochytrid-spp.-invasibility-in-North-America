...
mock_comm <- ps_mock  %>% #Create new object
  tax_glom(taxrank = "Genus", NArm=TRUE) %>% #Condense by genus      
...
############################ Rarefied dataset #################################################
#Run this code to evaluate the dataset rarefied to 90% of the minimum read length (https://micca.readthedocs.io/en/latest/phyloseq.html)
#We ran this code and evaluated the rarefied dataset compared to analysis of our raw reads. Significant results were the same, so we report results from raw reads. Therefore, we commented out this code, but remove '#' marks to rarefy the dataset and see results. 
#SB <- rarefy_even_depth(SB, rngseed=1, sample.size=0.9*min(sample_sums(SB)), replace=F)
#SB
################################################################################################

#Look at the top 15 phyla and genera for the dataset
...
#After adjustment none are significant
dunnTest(Simpson ~ Site, data = SB_meta, method = "bh")
#Sites NG+CD are significantly different from MC+DG in terms of dominant bacterial taxa in soil samples

#Testing by soil type
kruskal.test(Observed ~ Soil_Type, data = SB_meta)
kruskal.test(Simpson ~ Soil_Type, data = SB_meta)
#Both show significant differences, but where? Need to do Dunn's test.
#Dunn's test on diversity metrics for Soil Type
dunnTest(Observed ~ Soil_Type, data = SB_meta, method = "bh")
#After adjustment none are significant
dunnTest(Simpson ~ Soil_Type, data = SB_meta, method = "bh")
#Sites NG+CD are significantly different from MC+DG in terms of dominant bacterial taxa in soil samples
...
kruskal.test(Observed ~ Soil_Type, data = SB_meta_Bd)
kruskal.test(Simpson ~ Soil_Type, data = SB_meta_Bd)
#All p > 0.05
...
kruskal.test(Observed ~ Soil_Type, data = SB_meta_Bsal)
kruskal.test(Simpson ~ Soil_Type, data = SB_meta_Bsal)
#Bsal Simpson by site is significant. Run Dunn's test.
dunnTest(Simpson ~ Site, data = SB_meta_Bsal, method = "bh")
dunnTest(Simpson ~ Soil_Type, data = SB_meta_Bsal, method = "bh")
#CD significantly different from MC
...
########################################### RDA Analysis + PCoA ###############################
##This analysis is not included in results but leaving code here for future reference
#SB@sam_data
...
#All sites significantly different from one another
